...
def get_cached_categories():
    ...
    if (current_time - _category_cache['timestamp']) > _CATEGORY_CACHE_TTL:
        ...
        try:
            categories_docs = db.collection('categories').order_by('name').stream()
            _category_cache['data'] = [doc.to_dict()['name'] for doc in categories_docs if 'name' in doc.to_dict()]
            _category_cache['timestamp'] = current_time
        except Exception as e:
            app.logger.exception("Error refreshing category cache from Firestore.")
            # If refresh fails, return old data if available, or empty list
...
def products():
    ...
    products_query = db.collection('products').order_by('createdAt', direction=firestore.Query.DESCENDING)
    
    total_products_count = len(list(products_query.stream()))
...
def add_product():
    ...
    if request.method == 'POST':
        ...
        # Validate category against cached categories
        ...
        if imageUrl and not (imageUrl.startswith('http://') or imageUrl.startswith('https://')):
            errors.append('Image URL must be a valid URL (starting with http:// or https://).')

        if errors:
            for error in errors:
                flash(error, 'danger')
            return render_template('add_product.html', product=request.form, categories=categories) # Pass form data back to re-populate and categories
...
def edit_product(product_id):
    ...
    if request.method == 'POST':
        ...
        if errors:
            for error in errors:
                flash(error, 'danger')
            # Re-populate form with current product data and submitted changes
            product.update({
                'name': name,
                'price': price_str, # Keep as string for re-population
                'category': category,
                'description': description,
                'imageUrl': imageUrl,
                'inStock': inStock
